# YOLO 및 OpenCV 기반 실시간 객체 인식 프로젝트
...
일반적으로는 객체 인식이라고 표현하지만, 객체의 종류뿐 아니라 영상 속 위치까지 사각형으로 찾기 때문에 기술적으로는 **객체 검출(Object Detection)** 작업이다.
...
이 테스트는 주변에 형태가 비슷한 다른 차량을 함께 배치한 상태에서 학습 대상 객체가 구분되는지 확인한 정성적 실험이다.
